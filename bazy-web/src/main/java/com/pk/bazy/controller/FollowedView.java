...
@ManagedBean(name = "followedView")
@ViewScoped
public class FollowedView {
	...
	public void init() {
		...
		if (userName == null || userName.isEmpty()) {
			followers = session.getDao().getFollowed(session.getUser());
		} else {
			followers = session.getDao().getFollowed(new User(userName));
		}
